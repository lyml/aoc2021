...
int solve(int curr, int dest, bool extraChance) {
    if(curr == dest) return 1;
    auto & node = nodes[curr];
    bool visitAvailable = node.visits <= 0;
    if(!visitAvailable && (!extraChance || curr == 0)) return 0;
    bool nextraChance = !visitAvailable ? false : extraChance;
    node.visits += 1;
    int ret = 0;
    for(int next : node.edges) {
        ret += solve(next, dest, nextraChance);
    }
    node.visits -= 1;
    return ret;
}
